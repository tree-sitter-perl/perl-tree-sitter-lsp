...
2. **Reduce** via `ReducerRegistry` in `src/witnesses.rs`. Built-in reducers:
   ...
   - `BranchArmFold` — agreement across `BranchArm(InferredType)` per arm; needs ≥2 arms to claim. Single-arm ifs yield to the per-symbol writeback witness on `Symbol(sid)`. Claims `Variable` / `Symbol` / `Expression` attachments uniformly via `emit_branch_arm_witnesses`.
   - `FluentArityDispatch` — `ArityReturn { arg_count, return_type }` per arm. Walk classifies (Zero/Exact(N)/Default) but does NOT bake type — type comes from bag-aware fold in `resolve_return_types`. Only emitted for genuinely arity-discriminated subs. Claims `Symbol(_)`, `NamedSub(_)`, and `MethodOnClass{..}` attachments uniformly.
   - `NamedSubReturn` — `NamedSub(name)` attachment for cross-file imports. Pushed by enrichment; same path as locals. Slated to fold into `MethodOnClass` (D3).
   - `MethodOnClassReducer` — `MethodOnClass{class, name}` primary fallback; the class-keyed answer that disambiguates same-named methods on unrelated classes (`Sweet::flavor` vs `Sour::flavor`). Inheritance + cross-file bridges resolve via `query_rec`'s structural recursion on the same attachment shape (D3 will replace the structural walk with build-time `Edge(MethodOnClass(parent, name))` witnesses).
...
- `find_method_return_type(class, method, module_index, arity)` — class-keyed method return type. Thin wrapper that builds a `MethodOnClass{class, name}` `ReducerQuery`; the registry handles inheritance walks and cross-file bridges. Replaced the FA-side ancestor walker (D1).
...
6. `fold_to_fixed_point(chain_idx)` — **the worklist fold**. One driver replaces the old hand-ordered `fold → chain → fold → chain` sequence: each iteration runs `ChainTypingReducer::PreFold` (assignment + return-arm refresh) followed by `resolve_return_types` (the reducer-dispatch driver split out in Phase 4 — `ArityReturnReducer`, plugin-override seeding, `DelegationReducer`, `SelfMethodTailReducer`, `CallBindingPropagator`, hash-key-owner fixup). The loop exits when the snapshot of `Builder.resolved_returns` + `return_self_method` + `type_constraints.len()` stops moving; `MAX_FOLD_ITERATIONS = 64` is the debug-only safety net. (`resolved_returns` is build-only state — see `docs/prompt-cleanups.md` #1 for its planned removal in favor of direct bag queries.) After the lattice settles, `ChainTypingReducer::PostFold` runs once to fill `invocant_class` on `MethodCall` refs (`get_foo()->bar()` needs `get_foo`'s now-known return type).
