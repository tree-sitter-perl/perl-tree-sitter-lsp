...
- **One bag, one reducer, one worklist fold.** Walk pushes raw observations + `pending_witnesses` (branch arms / arity gates). After the walk, the build pipeline runs in fixed order (full sequence in `CLAUDE.md` "Build pipeline phases"): `apply_type_overrides` (pushes Plugin-priority Symbol witnesses) → `populate_witness_bag` (one-shot bag seed) → `Builder::fold_to_fixed_point` (the worklist driver). Each iteration of the driver runs `ChainTypingReducer::PreFold` (assignment + return-arm refresh) followed by `resolve_return_types` (which dispatches `ArityReturnReducer`, plugin-override seeding, `DelegationReducer`, `SelfMethodTailReducer`, `CallBindingPropagator`, hash-key-owner fixup); the loop exits when the snapshot of Sub/Method `return_type` + `type_constraints.len()` stops moving. `ChainTypingReducer::PostFold` runs once after the lattice settles to fill `MethodCall.invocant_class`. The two re-emittable passes inside `resolve_return_types` (`emit_arity_return_witnesses`, `propagate_call_bindings_to_constraints`) are clear-and-emit: they drop their prior bag/TC outputs at the start of every call, so the bag stays canonical regardless of how many iterations the driver runs. Plugin overrides dominate via `WitnessSource::priority()` (Plugin=100, Builder=10) — the `PluginOverrideReducer` short-circuit fires before the per-arm fold; no special-case skip in writeback.
- **Chain typing as one symbolic-execution function.** `Builder::resolve_invocant_class_tree` is THE recursive expression typer — used by chain-assignment typing, return-arm refresh, and method-call invocant filling, all driven by `ChainTypingReducer` inside the worklist loop. One typer, one reducer, three modes (assignment / return-arm / invocant). `FileAnalysis::resolve_expression_type` is the FA-side mirror used at query time. Adding new chain behavior means extending one of these two; new chain-typing helpers are forbidden.
...
Tests: 506 passing, 0 regressions across the spike. Pin-the-fix tests live in `witnesses::tests::*`, `file_analysis::tests::test_witness_*`, and `symbols::tests::test_demo_*` (the real-Mojolicious truth-table tests). `test_demo_chain_empirical_truth_table` is now positive on every link of `$r->get(...)->to(...)`. Worklist invariants pinned in `type_inference_invariants_tests`: deep-chain convergence (4-hop reassignment chain), plugin-override dominance, full re-fold idempotency (witness count + `type_constraints` length byte-identical across single-fold and double-fold runs), and provenance round-trip.
